...
class ZohoCreatorAPI:
    ...
    def post_attendance(
        ...
        try:
            # Always create the record via plain JSON POST — multipart variants
            # (both files-only and data+files mixed) cause Zoho Creator to return
            # code 3000 with a record ID but silently create nothing. Two-step is
            # the only reliable pattern: create record → upload file separately.
