...
# Override criteria: AWS must be this confident before approving a MiniFASNet rejection
_MIN_CONFIDENCE  = 90.0   # face detection confidence (0-100)
_MIN_SHARPNESS   = 40.0   # image sharpness (0-100)
_MIN_BRIGHTNESS  = 15.0   # image brightness (0-100)
...
def check_face_quality(image_bytes: bytes) -> dict:
    ...
    Returns a dict with:
      override  : True if AWS is confident enough to approve a MiniFASNet rejection
      detected  : whether any face was found
      confidence: AWS face detection confidence (0-100)
      sharpness : image sharpness score (0-100)
      brightness: image brightness score (0-100)
    ...
    try:
        ...
        return {"override": False, "detected": False, "reason": f"aws_error"}
    except Exception as e:
